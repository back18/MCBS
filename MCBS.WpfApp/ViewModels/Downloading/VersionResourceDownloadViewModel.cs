...
{
    [ExcludeFromDI]
    public partial class VersionResourceDownloadViewModel : ObservableObject, IDownloadViewModel, IDisposable
    {
        public VersionResourceDownloadViewModel(
            IScopeProvider scopeProvider,
            IDownloadViewModel versionJsonDownload,
            IDownloadCompletedHook<VersionJson> downloadCompletedHook,
            ILocalFileHook<VersionJson> localFileHook,
            IVersionResourceProvider resourceProvider,
            ...
            ILoggerFactory? loggerFactory)
        {
            ...
            ArgumentNullException.ThrowIfNull(versionJsonDownload, nameof(versionJsonDownload));
            ...
            _versionJsonDownload = versionJsonDownload;
            ...
            IsBusy = false;
            Progress = 0.0;
            StartCommand = new DependencyDownloadCommand(this, _lazyDownload, loggerFactory?.CreateLogger<VersionResourceDownloadViewModel>());
        ...
        private readonly IDownloadViewModel _versionJsonDownload;
        private readonly IDownloadCompletedHook<VersionJson> _downloadCompletedHook;
        private readonly ILocalFileHook<VersionJson> _localFileHook;
        private readonly IVersionResourceProvider _resourceProvider;
        ...
        public IDownloadViewModel? Dependent => _versionJsonDownload;
        ...
        public async Task<IDownload> LoadDownloadAsync()
        {
            ...
            try
            {
                ...
                if (_localFileHook.IsReady)
                    versionJson = await _localFileHook.LoadContentAsync();
                else if (_downloadCompletedHook.IsReady)
                    versionJson = await _downloadCompletedHook.LoadContentAsync();
                else
                    throw new InvalidOperationException("Neither local file nor download completed hook is ready.");

                DownloadAsset downloadAsset = _resourceProvider.GetDownloadAsset(versionJson);
