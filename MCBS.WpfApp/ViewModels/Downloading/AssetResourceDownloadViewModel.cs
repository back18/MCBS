...
{
    [ExcludeFromDI]
    public partial class AssetResourceDownloadViewModel : ObservableObject, IDownloadViewModel, IDisposable
    {
        public AssetResourceDownloadViewModel(
            string assetName,
            IScopeProvider scopeProvider,
            IDownloadViewModel assetManifestDownload,
            IDownloadCompletedHook<AssetManifest> downloadCompletedHook,
            ILocalFileHook<AssetManifest> localFileHook,
            IAssetResourceProvider resourceProvider,
            ...
            ILoggerFactory? loggerFactory)
        {
            ...
            ArgumentNullException.ThrowIfNull(assetManifestDownload, nameof(assetManifestDownload));
            ArgumentNullException.ThrowIfNull(downloadCompletedHook, nameof(downloadCompletedHook));
            ArgumentNullException.ThrowIfNull(localFileHook, nameof(localFileHook));
            ...
            _assetManifestDownload = assetManifestDownload;
            _downloadCompletedHook = downloadCompletedHook;
            _localFileHook = localFileHook;
            ...
            IsBusy = false;
            Progress = 0.0;
            StartCommand = new DependencyDownloadCommand(this, _lazyDownload, loggerFactory?.CreateLogger<AssetResourceDownloadViewModel>());
        ...
        private readonly IDownloadViewModel _assetManifestDownload;
        private readonly IDownloadCompletedHook<AssetManifest> _downloadCompletedHook;
        private readonly ILocalFileHook<AssetManifest> _localFileHook;
        private readonly IAssetResourceProvider _resourceProvider;
        ...
        public IDownloadViewModel? Dependent => _assetManifestDownload;

        public IDownload? DownloadTask => _download;

        public bool IsReady => _download is not null || _localFileHook.IsReady || _downloadCompletedHook.IsReady;
        ...
        public async Task<IDownload> LoadDownloadAsync()
        {
            ...
            try
            {
                ...
                if (_localFileHook.IsReady)
                    assetManifest = await _localFileHook.LoadContentAsync();
                else if (_downloadCompletedHook.IsReady)
                    assetManifest = await _downloadCompletedHook.LoadContentAsync();
                else
                    throw new InvalidOperationException("Neither local file nor download completed hook is ready.");

                DownloadAsset downloadAsset = _resourceProvider.GetDownloadAsset(assetManifest, _assetName);
